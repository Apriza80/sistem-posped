...
function BacksheetCoreGiro({ userActive }) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await API.get("/backsheet-core-giro");
      setRiwayatBacksheet(res.data.data ?? res.data);
    } catch (err) {
      console.error(
        err.response?.data?.message ||
          "Gagal mengambil data backsheet core giro",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      setUploading(true);
      await API.post("/backsheet-core-giro", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      alert("Dokumen Backsheet Core Giro System berhasil diunggah!");
  ...
  const handleHapus = async (id) => {
    ...
    try {
      await API.delete(`/backsheet-core-giro/${id}`);
  ...
  const handlePreview = async (item) => {
    try {
      const res = await API.get(`/backsheet-core-giro/${item.id}/preview`, {
        responseType: "blob",
      });
  ...
  const handleDownload = async (item) => {
    try {
      const res = await API.get(`/backsheet-core-giro/${item.id}/download`, {
        responseType: "blob",
      });
      const blobUrl = window.URL.createObjectURL(res.data);
  ...
  return (
    <div className="backsheet-page">
      <h1 className="page-title">Backsheet Core Giro System</h1>
          ...
          <div className="upload-form-row">
            ...
            <div className="form-group-field flex-grow-file">
              <div className="custom-file-wrapper">
                <label
                  htmlFor="file-upload-input-giro"
      ...
      <div className="card table-card-container">
        {loading ? (
          ...
          <div className="empty-state-msg">
            Belum ada data dokumen Backsheet Core Giro System yang tersimpan
            sesuai filter.
          </div>
        )}
      </div>
...
export default BacksheetCoreGiro;
