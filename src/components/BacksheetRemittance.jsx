...
import "./BacksheetPospay.css"; // Menggunakan file CSS yang sama agar styling dan responsifnya identik

function BacksheetRemittance({ userActive }) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await API.get("/backsheet-remittance");
      setRiwayatBacksheet(res.data.data ?? res.data);
    } catch (err) {
      console.error(
        err.response?.data?.message ||
          "Gagal mengambil data backsheet remittance",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      setUploading(true);
      await API.post("/backsheet-remittance", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      alert("Dokumen Backsheet Remittance berhasil diunggah!");
  ...
  const handleHapus = async (id) => {
    ...
    try {
      await API.delete(`/backsheet-remittance/${id}`);
  ...
  const handlePreview = async (item) => {
    try {
      const res = await API.get(`/backsheet-remittance/${item.id}/preview`, {
        responseType: "blob",
      });
  ...
  const handleDownload = async (item) => {
    try {
      const res = await API.get(`/backsheet-remittance/${item.id}/download`, {
        responseType: "blob",
      });
      const blobUrl = window.URL.createObjectURL(res.data);
  ...
  return (
    <div className="backsheet-page">
      {/* 1. HEADER HALAMAN */}
      <h1 className="page-title">Backsheet Remittance</h1>
          ...
          <div className="upload-form-row">
            ...
            <div className="form-group-field flex-grow-file">
              <div className="custom-file-wrapper">
                <label
                  htmlFor="file-upload-input-remit"
      ...
      <div className="card table-card-container">
        {loading ? (
          ...
          <div className="empty-state-msg">
            Belum ada data dokumen Backsheet Remittance yang tersimpan sesuai
            filter.
          </div>
        )}
      </div>
...
export default BacksheetRemittance;
