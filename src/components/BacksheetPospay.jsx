...
function BacksheetPospay({ userActive }) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await API.get("/backsheet-pospay");
      setRiwayatBacksheet(res.data.data ?? res.data);
    } catch (err) {
      console.error(
        err.response?.data?.message || "Gagal mengambil data backsheet",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      setUploading(true);
      await API.post("/backsheet-pospay", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      alert("Dokumen Backsheet Pospay berhasil diunggah!");
      setSelectedFile(null);
      fetchData(); // refresh riwayat dari server
    } catch (err) {
  ...
  const handleHapus = async (id) => {
    if (
      !window.confirm("Apakah Cece yakin ingin menghapus file backsheet ini?")
    )
      return;

    try {
      await API.delete(`/backsheet-pospay/${id}`);
  ...
  // Lihat file (preview) - lewat endpoint auth, ambil sebagai blob
  const handlePreview = async (item) => {
    try {
      const res = await API.get(`/backsheet-pospay/${item.id}/preview`, {
        responseType: "blob",
      });
  ...
  // Download file asli dari backend lewat endpoint auth
  const handleDownload = async (item) => {
    try {
      const res = await API.get(`/backsheet-pospay/${item.id}/download`, {
        responseType: "blob",
      });
      const blobUrl = window.URL.createObjectURL(res.data);
  ...
  return (
    <div className="backsheet-page">
      {/* 1. HEADER HALAMAN */}
      <h1 className="page-title">Backsheet Pospay</h1>
          ...
          <div className="upload-form-row">
            ...
            <div className="form-group-field flex-grow-file">
              <div className="custom-file-wrapper">
                <label htmlFor="file-upload-input" className="btn-browse-file">
                  📎 Telusuri
                </label>
                <input
                  type="file"
      ...
      <div className="card table-card-container">
        {loading ? (
          ...
          <div className="empty-state-msg">
            Belum ada data dokumen Backsheet Pospay yang tersimpan sesuai
            filter.
          </div>
        )}
      </div>
...
export default BacksheetPospay;
